...
| `RECOV-33` — client-identity header step (Append/Replace token composition, blank-line suppression) | Phase 5a brainstorm | **Phase 7** | One of only two appendix-C `RECOV-17`–`RECOV-34` rows with no `§9` `RETRY-*` twin (the other, `RECOV-32`'s idempotency key, shipped in 5a because retry preserves it per `RETRY-38`). Pure configuration-driven header composition with zero retry coupling, so it travels with `CFG-*` |
| `StepContext.signal` — exposing the call's `AbortSignal` to steps | Phase 5a brainstorm | **Phase 5a, Task 1** | Found during 5a's spec self-review: 4c's `Cursor` accepts and threads a `signal` but `StepContext` never exposed it, so no step could observe cancellation — `RETRY-26`'s cancellable wait and `RETRY-32`'s "no attempts after cancellation" were both unimplementable. Additive one-field amendment, landed with its first consumer rather than patched into 4c's already-reviewed plan. 5b's redirect loop and 5c's token fetch need the same field |
...
| `standardResilience()` gains a `LOGGING` pillar step | Phase 5c brainstorm | Phase 7 | 5c's preset installs only the three pillars that exist by then (redirect, retry, auth); a real logging step doesn't exist until Phase 7 (Instrumentation & Configuration). Documented as a scope boundary in 5c's design, not an omission |
| `DigestChallengeUnsupportedError` — confirm a real caller-facing API needs to distinguish "unsatisfiable challenge" from "no replacement" before shipping it | Phase 5c brainstorm | **Phase 9** (kept at plan time, on notice) | `authStep()` itself never surfaces this distinction (both cases just leave the 401 unchanged); the leaf was sketched for a lower-level API 5c's design did not otherwise build. 5c's plan **kept** it rather than cutting — as an `@internal` leaf for a caller composing `composingHandler`/`digestHandler` directly, bypassing `authStep()`. That consumer is hypothetical: if Phase 9's conformance sweep finds none, cut it then rather than shipping it into a public surface |
| Basic/Digest never stamp preemptively — an *interpretation*, not a stated requirement | Phase 5c brainstorm | **Phase 9** (conformance sweep) | `§11` phrases `AUTH-14` and `AUTH-15`–`AUTH-22` entirely as reactions to a parsed challenge, and never describes a preemptive-Basic path the way it separately describes Bearer's preemptive cached-token stamp; Digest structurally cannot stamp before seeing `realm`/`nonce`. 5c treats both uniformly as challenge-only. If Phase 9 turns up reference fixtures showing preemptive Basic, this is the reading to revisit — the change is confined to `auth-step.ts`'s per-scheme outbound branch |
| True per-call / per-operation `AuthTiers`, resolved from `ExecutionContext` rather than fixed at step construction | Phase 5c plan | **Unscoped** — no phase in this roadmap wires it | `AUTH-4`'s three tiers (`perCall`/`operation`/`client`) presuppose three genuinely different lookup sources, but no phase — 4a's `ExecutionContext` included — ships a per-call auth-requirement extension point to read them from. 5c's `AuthStepSettings.tiers` is therefore one static value fixed at `authStep()` construction; `resolveAuthRequirement`'s own logic (`AUTH-4`–`AUTH-7`) is mechanically satisfied either way, so this is a plumbing gap, not a conformance one. Whichever phase first gives `ExecutionContext` a caller-populated extension slot should close it |
| Redirect predicate's scope over safety mechanics (credential stripping, downgrade, replayability, loop/cap) — 5b reads `REDIR-20`'s "MUST fully override" as scoped to code/method eligibility only, not these | Phase 5b brainstorm | Re-confirm at Phase 9 conformance sweep, or sooner if the user disagrees | A judgment call made without the user present; 5b's own design flags it as narrow and mechanical to reverse if wrong |
| Redirect structured logging (`SHOULD`-level hop/loop/downgrade events) | Phase 5b brainstorm | Phase 7 | Same disposition as 5a's equivalent gap for retry — `Logger`/`LogEvent` seam not built until Phase 7 |
