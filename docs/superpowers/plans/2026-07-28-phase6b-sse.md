...
export class SseLineReader {
  ...
  async nextLine(): Promise<string | typeof SSE_END> {
    ...
    for (;;) {
      ...
      if (byte === undefined) {
        // EOF. A held CR already terminated its own line on the previous call, so it contributes nothing here —
...
test('await using releases the stream exactly once, where the runtime supports it (styleguide 13.1/13.2)', async () => {
  const {stream, closes} = streamOver('data: a\n\ndata: b\n\n');
...

  {
    ...
    for await (const _event of scoped) break;
  }
  expect(closes()).toBe(1);
...
export function sseStreamFrom(
  ...
  const source = new BufferedSource(body.getReader());
  const parser = new SseParser(source, {maxLineBytes: options?.maxLineBytes});
  return new SseStream(parser, closingBoth(source, response), options);
}
