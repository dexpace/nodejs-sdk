...
## Conflicts
- **design vs styleguide: package manager and lockfile** — the design specifies a pnpm workspace with the `catalog:` protocol as the single place version coordinates live; the styleguide mandates `bun install` against a committed `bun.lock` with Bun pinned via `.bun-version`, and names `bun install --frozen-lockfile` as the CI gate. These are two different package managers and two different lockfiles for the same repo.
  <sub>design `docs/sdk-design-nodejs/02-package-and-workspace-layout.md:50-51` · design `docs/sdk-design-nodejs/09-toolchain-and-quality-gates.md:15` · styleguide `/home/mohammad/Projects/dexpace/styleguide/typescript/01-formatting-and-tooling.md:112-120` · unresolved 2026-07-25</sub>
- **design vs styleguide: test runner and whether coverage gates the build** — the design specifies `c8`/`@vitest/coverage-v8` with a `coverage.thresholds` aggregate floor wired into the default `test` script; the styleguide mandates `bun test` with symbols imported from `bun:test`, and states coverage is reported as a trend and a build never passes or fails on a coverage number alone. The disagreement is twofold: which runner, and whether a coverage threshold may gate CI at all.
  <sub>design `docs/sdk-design-nodejs/09-toolchain-and-quality-gates.md:12` · styleguide `/home/mohammad/Projects/dexpace/styleguide/typescript/11-testing.md:47-48` · styleguide `/home/mohammad/Projects/dexpace/styleguide/typescript/11-testing.md:210-213` · unresolved 2026-07-25</sub>
- **design vs styleguide: gts as the lint and format baseline** — the styleguide requires gts as the sole toolchain, with exactly one ESLint overlay extending it and every Prettier default final and non-overridable; the design's toolchain table specifies typescript-eslint's `strict-type-checked` and `stylistic-type-checked` directly and never mentions gts, so the port as designed has no gts baseline to extend.
  <sub>design `docs/sdk-design-nodejs/09-toolchain-and-quality-gates.md:8-10` · styleguide `/home/mohammad/Projects/dexpace/styleguide/typescript/01-formatting-and-tooling.md:66-83` · styleguide `/home/mohammad/Projects/dexpace/styleguide/typescript/01-formatting-and-tooling.md:122-129` · unresolved 2026-07-25</sub>
