...
## G. Phase 7a — Configuration & Platform Primitives

### G1 — `clientIdentityStep` is not reachable from the public barrel — **SCHEDULED** (Phase 5c)
...
### G2 — Proxy resolution implements the property tier the design doc ledgered as collapsed — **RESOLVED** (2026-08-27)
...
### G3 — `CFG-12` is documented, not enforced — **WATCH**
...
### G4 — `CFG-28`'s global-configuration convenience resolver is not built — **DECIDE**
...
### G5 — `CFG-35`'s throwable axis is not in this phase — **SCHEDULED** (Phase 5a)
...
### G6 — Six defects in the phase plan's own implementation sketches — **WATCH**
...
### G7 — `eslint.config.js`'s Node-globals block was widened — **WATCH**
...
### G8 — `src/generated/version.ts` is committed and can be stale — **WATCH**
...
### G9 — `Configuration.default()` ships as a free `defaultConfiguration()` — **RESOLVED** (2026-08-27)
...
### G10 — `CFG-24`'s warning half is not emitted — **SCHEDULED** (Phase 7b)
...
### G11 — `client-identity-step.ts`'s folder placement is provisional — **SCHEDULED** (with G1)
...
surface-widening the phase declined to do for G1:
...
1. **It is the sole outbound `config/ → pipeline/` edge.** Three of its four imports leave the folder —
   ...
   return edge, and nothing in CI would catch the loop (see G12).
...
name. **Trigger:** the same phase as G1 — whichever one promotes the pipeline authoring surface. Settle the
file's folder and the barrel question together there, and record the corpus tension at that point.

### G12 — No import-cycle gate exists in CI — **WATCH** (repo tooling, not this phase)
...
the next phase that touches the CI gate list, or the first observed cycle — G11's `config/ → pipeline/` edge
being the nearest candidate.

### G13 — A `CFG-7`-valid duration can exceed what any timer can honor — **SCHEDULED** (first phase wiring a configured duration into a timer)
...
### G14 — A configuration seam that fails is now silently invisible — **SCHEDULED** (Phase 7b, with G10)
...
nothing anywhere to say why. The same shape as G10, the same cause, and the same blocker: there is no
`Logger` in the package until 7b ships the `OBS-*` seam. **Trigger:** when 7b's `Logger` lands, wire
`readLayer`'s `catch` to it **together with** G10's three null paths in `config/proxy.ts` — they are one
change, not two.

### G15 — `HTTP-17`: `hasForbiddenNameByte` permits a space inside a header name — **DECIDE** (Phase 1 code, surfaced by 7a)
...
### G16 — `deepEqual` / `deepHash` require acyclic, bounded-depth input — **WATCH**
...
### G17 — `formatProxyOptions` re-brackets an IPv6 host that `ProxyOptions.host` stores bare — **RESOLVED** (2026-08-27)
...
### G18 — `isHeaderSafe` duplicates `http/ascii-validation.ts`'s outbound byte predicate — **WATCH** (split out of G11, 2026-08-27)
...
already a live concern (G11 tracks the `config/ → pipeline/` one), and adding a second one to reuse four
lines is the wrong side of that trade while G12's import-cycle gate does not exist.

Recorded separately from G11 because G11's own resolution — settle `client-identity-step.ts`'s folder and the
...
### G19 — No `fast-check` property test logs its seed — **WATCH** (repo tooling, not this phase)
